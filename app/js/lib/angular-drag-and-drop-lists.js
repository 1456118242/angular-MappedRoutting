        ...
        return function(scope, element, attr) {
          ...
          var getOffsetTop = function (element) {
            ...
            while (parentElement = parentElement.offsetParent) {
              offsetTop += !isNaN(parentElement.offsetTop) ? parentElement.offsetTop : 0;
            }
          ...
          element.on('dragover', function(event) {
            ...
            try {
                var offsetTop = getOffsetTop(listNode);

                if((window.innerHeight - (event.pageY - window.pageYOffset))<30){
                window.scrollBy(0,2);
                }
                if((event.pageY - window.pageYOffset)<30 && (event.pageY - window.pageYOffset)>0){
                window.scrollBy(0,-2);
                }

              if ( event.pageY - offsetTop < 50 && event.pageY - offsetTop > 0 ) {
                listNode.scrollTop = listNode.scrollTop - 2;
              }

              if ( (listNode.offsetHeight + offsetTop - event.pageY) < 50 && (listNode.offsetHeight + offsetTop - event.pageY) > 0 ) {
                listNode.scrollTop = listNode.scrollTop + 2;
              }
            }catch(e) {
              console.log("dragover error");
            }
